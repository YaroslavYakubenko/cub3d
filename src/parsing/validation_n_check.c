...
/*   Updated: 2025/03/08 19:32:24 by yyakuben         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_spaces(char **map, t_game *game)
{
	...
	while (map[++y])
	{
		...
		while (map[y][++x])
		{
			if (map[y][x] == '0')
			{
				if ((int)ft_strlen(map[y - 1]) <= x || (int)ft_strlen(map[y]) <= x || (int)ft_strlen(map[y + 1]) <= x)
					error_for_walls(game);
				if (map[y - 1][x - 1] == ' ' || map[y - 1][x] == ' ' || map[y - 1][x + 1] == ' ' || map[y][x + 1] == ' ' || map[y][x - 1] == ' ' || map[y + 1][x - 1] == ' ' || map[y + 1][x] == ' ' || map[y + 1][x + 1] == ' ')
					error_for_walls(game);
			}
		}
	}
...
void check_end(char **map, t_game *game)
{
	...
	while (map[y])
	{
		...
		while (map[y][x])
		{
			if (map[y][x] == '0')
			{
				if (map[y - 1][x + 1] == '\0' || map[y][x + 1] == '\0' || map[y + 1][x + 1] == '\0')
				{
					printf("Error: problems with walls.\n");
					free_map(game->map);
					free(game);
